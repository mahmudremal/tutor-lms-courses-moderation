        ...
        <div class="card-body">
          <div class="fancy-table table-responsive border rounded">
            <table class="table table-striped mb-0">
              <thead>
                <tr>
                  <th scope="col"><?php esc_html_e( 'Email', 'tutor-lms-courses-moderation' ); ?></th>
                  <th scope="col"><?php esc_html_e( 'Profiles', 'tutor-lms-courses-moderation' ); ?></th>
                  <th scope="col"><?php esc_html_e( 'Client Status', 'tutor-lms-courses-moderation' ); ?></th>
                  <th scope="col"><?php esc_html_e( 'Meeting Time & link', 'tutor-lms-courses-moderation' ); ?></th>
                  <th scope="col"><?php esc_html_e( 'Retainer Amount', 'tutor-lms-courses-moderation' ); ?></th>
                  <!-- Contract Options -->
                  <th scope="col"><?php esc_html_e( 'Status', 'tutor-lms-courses-moderation' ); ?></th>
                  <th scope="col"><?php esc_html_e( 'Action', 'tutor-lms-courses-moderation' ); ?></th>
                </tr>
              </thead>
              <tbody>
                <?php if( count( $subscribers ) <= 0 ) : ?>
                  <tr>
                    <td colspan="7"><img src="<?php echo esc_url( TUTOR_LMS_COURSES_MODERATION_SCRATCH_BUILD_URI . '/icons/not-found.svg' ); ?>" alt="<?php esc_attr_e( 'Noting found', 'tutor-lms-courses-moderation' ); ?>" style="width: 100%;height: auto;max-height: 400px;"></td>
                  </tr>
                <?php endif; ?>
                <?php foreach( $subscribers as $subscriber ) :
                  ...
                  if( $doc && ! empty( $doc->document_status ) ) {$is_signed = ( $doc->document_status == 'signed' );} else {$is_signed = null;}
                    //   Missing
                  $is_signed = ( $is_signed === null ) ? __( 'Pending', 'tutor-lms-courses-moderation' ) : (
                    ( $is_signed === true ) ? __( 'Signed', 'tutor-lms-courses-moderation' ) : __( 'Pending', 'tutor-lms-courses-moderation' )
                  );
                  $is_active = $activeSubscription ? __( 'Active', 'tutor-lms-courses-moderation' ) : __( 'Paused', 'tutor-lms-courses-moderation' );
                    ...
                    <td>
                      <div class="d-flex align-items-center">
                        <div class="media-support-info">
                          ...
                          <a class="mb-0" href="<?php echo esc_url( $userInfo->meta->meeting_link ); ?>" target="_blank">@<?php esc_html_e( 'Meeting link', 'tutor-lms-courses-moderation' ); ?></a>
                          <?php endif; ?>
                        </div>
                      </div>
                    </td>
