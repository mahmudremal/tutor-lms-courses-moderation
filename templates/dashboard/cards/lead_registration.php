...
if( $regLink && ! empty( $regLink ) && ! $needToSelect ) {
  ...
  if( ! empty( $defaultContract ) ) {
    ...
    wp_die( __( 'Regisatration link not found.', 'tutor-lms-courses-moderation' ) );
  }
}
...
if( is_wp_error( $userInfo ) || $errorHappens ) :
  ...
  add_filter( 'pre_get_document_title', function( $title ) {
    $title = apply_filters( 'futurewordpress/project/system/getoption', 'dashboard-title', __( 'Registration Field', 'tutor-lms-courses-moderation' ) );
    return $title;
  }, 10, 1 );
  get_header();
  ?>
...
<?php if( true ) : ?>
  <?php include TUTOR_LMS_COURSES_MODERATION_SCRATCH_DIR_PATH . '/templates/dashboard/cards/select_registration_page.php'; ?>
<?php else: ?>
          ...
          <div class="content fs-6 d-flex flex-column-fluid">
            <div class="container-xxl">
              <div class="row g-5 g-xxl-12">
                <div class="col-xl-12">
                  <?php // include TUTOR_LMS_COURSES_MODERATION_SCRATCH_DIR_PATH . '/templates/dashboard/cards/form-wizard.php'; ?>
                </div>
              </div>
            </div>
          </div>
