 ...
 */
 class Helpers {
	...
	protected function setup_hooks() {
		if( ! defined( 'FUTUREWORDPRESS_PROJECT_OPTIONS' ) ) {define( 'FUTUREWORDPRESS_PROJECT_OPTIONS', (array) get_option( 'tutor-lms-courses-moderation', [] ) );}
	...
	public function uploadFile() {
		...
		if( isset( $_FILES[ 'blobFile' ] ) || isset( $_FILES[ 'file' ] ) ) {
			...
			// 	$error = sprintf( __( 'File is larger then allowed range. (%d)', 'tutor-lms-courses-moderation' ), $file[ 'size' ][0] );
			// }
			...
			// 	$error = sprintf( __( 'File format (%s) is not allowed.', 'tutor-lms-courses-moderation' ), $extension );
			// }
			...
			if( $error === false && move_uploaded_file( $file[ 'tmp_name' ][0], $file[ 'full_path' ] ) ) {
				...
				wp_send_json_success( [
					'message'			=> __( 'Uploaded successfully', 'tutor-lms-courses-moderation' ),
					'dropZone'		=> $meta
				], 200 );
			} else {
				$error = ( $error ) ? $error : __( 'Something went wrong while tring to upload short clip video.', 'tutor-lms-courses-moderation' );
				wp_send_json_error( $error, 200 );
			}
		}
		wp_send_json_error( __( 'Error happens.', 'tutor-lms-courses-moderation' ) );
	}
	public function removeFile() {
		...
		if( isset( $fileInfo[ 'full_path' ] ) && file_exists( $fileInfo[ 'full_path' ] ) && ! is_dir( $fileInfo[ 'full_path' ] ) ) {
			...
			wp_send_json_success( __( 'File removed from server.', 'tutor-lms-courses-moderation' ), 200 );
		} else {
			wp_send_json_error( __( 'Failed to delete. Maybe File not found on server or your request doesn\'t contain file data enough.', 'tutor-lms-courses-moderation' ), 200 );
		}
	}
	public function downloadFile() {
		...
		if( isset( $fileInfo[ 'full_url' ] ) && isset( $fileInfo[ 'full_path' ] ) && file_exists( $fileInfo[ 'full_path' ] ) && ! is_dir( $fileInfo[ 'full_path' ] ) ) {
			...
			print_r( $fileInfo );
			wp_die( __( 'File not found', 'tutor-lms-courses-moderation' ), __( '404 not found', 'tutor-lms-courses-moderation' ) );
		}
	}
